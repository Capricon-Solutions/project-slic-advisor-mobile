...
export default function Competition({ navigation }) {
  ...
  const filterCompetitionsByDate = (data) => {
    ...
    const filterFunc = (items) =>
      items?.filter((item) => {
        const yearMatch = selectedYear ? item.year.toString() === selectedYear : true;
        const monthMatch = selectedMonth ? item.month.toString() === selectedMonth : true;
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 15 }}>
          {[
            ...
            <TouchableOpacity
              ...
              style={{
                backgroundColor: SelectedType === item.id ? COLORS.primary : COLORS.white,
                borderRadius: 15,
                flex: 1,
                marginHorizontal: 5,
                justifyContent: 'center',

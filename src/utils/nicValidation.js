...
const validateSriLankanNIC = nic => {
  ...
  if (oldFormat.test(nic)) {
    ...
    return {
      isValid: false,
      error:
        'Invalid NIC format. Use old format (9 digits + V/X) or new format (12 digits)',
    };
  }
  ...
  if (actualDayOfYear > maxDaysInYear) {
    return {
      isValid: false,
      error: `Day ${actualDayOfYear} does not exist in year ${birthYear} (${
        isLeapYear(birthYear) ? 'leap' : 'non-leap'
      } year)`,
    };
  }
